<template>
  <div class="pt-20">
    <section id="contact" class="bg-white">
      <div class="max-w-7xl mx-auto px-8 lg:px-12 py-20 lg:py-24">
        <div class="grid lg:grid-cols-2 gap-16 lg:gap-20">
          <div>
            ...
            <p class="text-xl text-ink/50 leading-relaxed mb-12">
          ...
          <div class="bg-surface-soft rounded-3xl p-10 lg:p-12">
            <h3 class="text-3xl font-bold text-ink mb-8">发送咨询</h3>
            <form @submit.prevent="submitForm" class="space-y-6">
              <div>
                <label class="block text-base font-medium text-ink/60 mb-3">您的姓名</label>
                <input v-model="form.name" type="text" required
                       class="w-full px-5 py-4 bg-white rounded-xl border border-hairline/50 text-lg text-ink placeholder-ink/30 focus:outline-none focus:border-primary/50 transition-colors"
                       placeholder="张先生">
              </div>
              <div>
              ...
              <div>
                ...
                <textarea v-model="form.message" rows="4" required
                          class="w-full px-5 py-4 bg-white rounded-xl border border-hairline/50 text-lg text-ink placeholder-ink/30 focus:outline-none focus:border-primary/50 transition-colors resize-none"
                          placeholder="请描述您的需求..."></textarea>
              </div>
              <button type="submit" :disabled="loading"
                      class="w-full py-4 bg-ink text-white text-lg font-medium rounded-full hover:bg-ink/90 transition-all disabled:opacity-50 disabled:cursor-not-allowed">
                {{ loading ? '提交中...' : '提交咨询' }}
              </button>
              <p v-if="message" class="text-center py-3 px-4 rounded-xl text-base" :class="message.type === 'success' ? 'bg-block-mint/30 text-emerald-600' : 'bg-block-pink/30 text-red-500'">
                {{ message.text }}
              </p>
            </form>
          </div>
...
export default {
  ...
  data() {
    return {
      ...
      message: '',
      contacts: [
        { label: '联系电话', value: '400-000-0000', iconPath: 'M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z' },
        { label: '电子邮箱', value: 'contact@example.com', iconPath: 'M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z' },
